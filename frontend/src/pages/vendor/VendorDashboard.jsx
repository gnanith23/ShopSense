...
export default function VendorDashboard() {
  ...
  async function handleDeleteProduct() {
    ...
    try {
      ...
      const updated = products.filter((p) => p._id !== deleteTarget.id);
      setProducts(updated);
      setRecentProducts(updated.slice(0, 5));
    ...
    <div>
      ...
      {products.length > 0 && (
        <div style={{ marginBottom: '1.5rem' }}>
          <AIInsightsWidget products={products} />
        </div>
      )}

      {/* AI Assistant + Catalog Quality row */}
      {products.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '1.5rem', marginBottom: '1.5rem', alignItems: 'stretch' }}>
          <AIAssistantPanel products={products} analytics={analytics} />
          <CatalogQualityScore products={products} />
        </div>
      )}
